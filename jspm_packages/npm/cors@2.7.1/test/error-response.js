...
(function() {
  ...
  describe('error response', function() {
    it('500', function(done) {
      supertest(app).post('/five-hundred').expect(500).end(function(err, res) {
    ...
    it('401', function(done) {
      supertest(app).post('/four-oh-one').expect(401).end(function(err, res) {
        should.not.exist(err);
        res.headers['access-control-allow-origin'].should.eql('*');
    ...
    it('404', function(done) {
      supertest(app).post('/four-oh-four').expect(404).end(function(err, res) {
        should.not.exist(err);
        res.headers['access-control-allow-origin'].should.eql('*');
        done();
      });
